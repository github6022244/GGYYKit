...
- (void)_displayAsync:(BOOL)async {
    ...
    if (async) {
        ...
        // 捕获必要属性（避免 block 中访问 self）
        CGColorRef backgroundColorRef = NULL;
        if (opaque && self.backgroundColor) {
            backgroundColorRef = CGColorRetain(self.backgroundColor);
        }
        ...
        dispatch_async(YYAsyncLayerGetDisplayQueue(), ^{
            if (isCancelled()) {
                if (backgroundColorRef) CGColorRelease(backgroundColorRef);
                return;
            }

            UIImage *image = [renderer imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull ctx) {
                CGContextRef context = ctx.CGContext;
                if (opaque && context && backgroundColorRef) {
                    CGContextSaveGState(context);
                    {
                        if (CGColorGetAlpha(backgroundColorRef) < 1) {
            ...
            if (isCancelled()) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (task.didDisplay) task.didDisplay(self, isCancelled() ? NO : YES);
                });
            }
        ...
        UIImage *image = [renderer imageWithActions:^(UIGraphicsImageRendererContext * _Nonnull ctx) {
            CGContextRef context = ctx.CGContext;
            if (opaque && context && backgroundColorRef) {
                CGSize renderSize = CGSizeMake(size.width * scale, size.height * scale);
                CGContextSaveGState(context);
                {
                    if (CGColorGetAlpha(backgroundColorRef) < 1) {
                        CGContextSetFillColorWithColor(context, [UIColor whiteColor].CGColor);
                        CGContextAddRect(context, CGRectMake(0, 0, renderSize.width, renderSize.height));
                        CGContextFillPath(context);
                    }
                    CGContextSetFillColorWithColor(context, backgroundColorRef);
                    CGContextAddRect(context, CGRectMake(0, 0, renderSize.width, renderSize.height));
